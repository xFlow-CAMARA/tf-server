...
##
# This file is part of the Open SDK, based on sunrise6g_opensdk.network.core.schemas.py
#
# Contributors:
#
...
class AsSessionWithQoSSubscription(BaseModel):
    ...
    def subscription_id(self) -> str:
        ...
        if not subscription_id:
            ...
            raise OranPlatformError("QoS session ID not found in response")
...
# ORAN Policy models (client-side request)
class OranPolicy(BaseModel):
    policy_type: str = Field(..., alias="policyType")
    policy_scope: dict = Field(..., alias="policyScope")
    policy_statement: dict = Field(..., alias="policyStatement")
    expiry: Union[datetime, int, None] = None
    notification_uri: AnyHttpUrl | None = Field(None, alias="notificationUri")
